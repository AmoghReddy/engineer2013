...
def event_sidebar(event,i):
	...
	for j in tab_type:
		if (j != "image" and j != "contact"):
			print "<span id='blockGen"+get_num()+"' style='width:240px;padding: 3px;padding-left: 5px;padding-right: 5px;cursor: pointer;"+get_color()+"' onclick='getPage(\""+event+"\",\""+event+'_'+j+'\")\'><h3>'+j.replace('_',' ')+"</h3></span>"
		# print tab_type
		# print event
		# if event == "Proteus":
		# print aiufibof
			# print j
			# print lqn;
	print "<span id='blockGen"+get_num()+"' style='width:240px;padding: 3px;"+get_color()+"'>"+"<h3>Contact: "+events[i][event]['contact']+"</h3></span>"
	print "</span>"
	print "</td>"
