...
{
	this.name = element.id.substr(5);
	this.DOMobject = element;
	this.target = { page: undefined, sphere: undefined, helix: undefined, grid: undefined };
	this.WGLobject;
...
function page( element )
{
	...
	{
		var children = [];
		getAllChildren(this.pageElement, REblock, children);
		for ( var i = 0; i < children.length; i++ )
		{
			var blk = new block(children[i]);
			this.blocks[blk.name] = blk;
		}
		getPageTargets(this.name);
		getSphereTargets(this.name, 900);
		getHelixTargets(this.name, 1100);
		getGridTargets(this.name);
		loadWGLObjects(this.name);
...
function getPageTargets(name)
{
	...
	for ( i in allPages[name].blocks )
	{
		...
		allPages[name].blocks[i].initialPosition = pos;
		...
		allPages[name].blocks[i].target.page =  object;
	}
}

function getNextScroll(name, destination, alpha)
{
	var vect = new THREE.Vector3();
	vect.copy( pivot );
	...
	{
		object = new THREE.Object3D();
		vect.x = allPages[name].blocks[i].target.page.position.x;
		theta = (allPages[name].blocks[i].initialPosition.y / vect.z) + alpha;
		object.position.x = allPages[name].blocks[i].target.page.position.x;
		object.position.y = vect.z * Math.sin(theta);
		object.position.z = vect.z - vect.z * Math.cos(theta);
		lookAwayFrom(object, vect);
...
function getSphereTargets(name, radius)
{
